...
/*
 ...
 // For example, if an app has 2 screens and user is moving from screen 1 to screen 2. We only want to
    allocate screen 2 WHEN we need it (ie. when the user clicks a button to segue to screen 2). When
    we get to screen 2, we may want to deallocate screen 1.
 ...
 // When you edit a Reference type, you edit the object that you are referencing. This "reference" is what
 called "pointer" because it "points" to an object in the Heap (in memory).
...
// During the day, the teacher will hand out many different quizzes to differente classes. The students
// will answer the quizzes and return them back to the teacher
...
// In this example, there are many different types of quizzes. The teacher hands out the quizzes and the
// the students take the quizzes and return them to the teacher.
...
// Objects that we create and want to perform action inside
